...
{{-- HARİTA — Admin → Ayarlar → "Google Harita Embed" → "Ofis Harita Embed URL" --}}
<section class="mx-auto max-w-6xl px-5 pb-16 lg:pb-20">
    @if(!empty($settings['map_office_embed']))
        @if(!empty($settings['map_office_title']))
            <h2 class="mb-4 text-2xl font-extrabold tracking-tight text-ink">{{ $settings['map_office_title'] }}</h2>
        @endif
        <div class="overflow-hidden rounded-2xl ring-1 ring-hair">
            <iframe src="{{ $settings['map_office_embed'] }}" class="h-72 w-full lg:h-96" style="border:0" allowfullscreen loading="lazy" referrerpolicy="no-referrer-when-downgrade" title="{{ $settings['map_office_title'] ?? __('Harita') }}"></iframe>
        </div>
        @if(!empty($settings['map_factory_embed']))
            @if(!empty($settings['map_factory_title']))
                <h2 class="mb-4 mt-10 text-2xl font-extrabold tracking-tight text-ink">{{ $settings['map_factory_title'] }}</h2>
            @endif
            <div class="mt-4 overflow-hidden rounded-2xl ring-1 ring-hair">
                <iframe src="{{ $settings['map_factory_embed'] }}" class="h-72 w-full lg:h-96" style="border:0" allowfullscreen loading="lazy" referrerpolicy="no-referrer-when-downgrade" title="{{ $settings['map_factory_title'] ?? __('Fabrika Haritası') }}"></iframe>
            </div>
        @endif
    @else
        {{-- Embed girilmemişse stilize placeholder --}}
        <div class="relative grid h-72 place-items-center overflow-hidden rounded-2xl bg-leaf-500/5 ring-1 ring-hair lg:h-96">
            <div class="flex flex-col items-center text-center">
                ...
                <span class="mt-3 rounded-lg bg-white px-4 py-2 text-sm font-bold text-ink shadow-sm">{{ $settings['site_name'] ?? 'Unikeyterra' }}@if($settings['contact_city'] ?? null) · {{ $settings['contact_city'] }}@endif</span>
            </div>
        </div>
    @endif
</section>
