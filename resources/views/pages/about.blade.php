...
<div class="hero-band bg-earth-600">
    <div class="mx-auto max-w-6xl px-5 pb-16 pt-4 lg:pb-24 lg:pt-8">
        ...
        <span class="text-sm font-bold uppercase tracking-[0.12em] text-leaf-300">{{ $aboutHeroEyebrow }}</span>
        <h1 class="mt-3 max-w-3xl text-[clamp(2.2rem,4.5vw,3.4rem)] font-medium leading-[1.08] tracking-tight text-white">{{ $aboutHeroTitle }}</h1>
    </div>
</div>
...
<section class="mx-auto max-w-6xl px-5 py-16 lg:py-20">
    <div class="grid grid-cols-1 items-center gap-10 lg:grid-cols-2 lg:gap-14">
        <div class="overflow-hidden rounded-2xl">
            <img src="{{ $aboutStoryImage }}" alt="{{ $brandName }}" class="aspect-[4/3] w-full object-cover" loading="lazy" decoding="async">
        </div>
        <div>
            <span class="text-sm font-bold uppercase tracking-[0.12em] text-leaf-600">{{ $aboutStoryEyebrow }}</span>
            <h2 class="mt-2 text-3xl font-extrabold tracking-tight text-ink lg:text-4xl">{{ $aboutStoryTitle }}</h2>
            @if($aboutStoryBody)
                @foreach(preg_split('/\n\s*\n/', trim($aboutStoryBody)) as $para)
                    <p class="mt-4 text-[16px] leading-relaxed text-ink-soft">{!! nl2br(e(trim($para))) !!}</p>
                @endforeach
...
<section class="bg-leaf-500/5 py-16 lg:py-20">
    <div class="mx-auto max-w-6xl px-5">
        <div class="mb-10 text-center">
            <span class="text-sm font-bold uppercase tracking-[0.12em] text-leaf-600">{{ $aboutValuesEyebrow }}</span>
            <h2 class="mt-2 text-3xl font-extrabold tracking-tight text-ink lg:text-4xl">{{ $aboutValuesTitle }}</h2>
        </div>
        <div class="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
            @foreach($aboutValues as $i => $v)
                <div class="rounded-2xl bg-white p-7 ring-1 ring-hair">
                    <span class="grid h-14 w-14 place-items-center rounded-xl bg-leaf-500/10 text-leaf-600"><svg class="h-7 w-7" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" aria-hidden="true"><path d="{{ $valueIcons[$i % count($valueIcons)] }}"/></svg></span>
                    <h3 class="mt-5 text-lg font-extrabold text-ink">{{ $v['title'] ?? '' }}</h3>
                    <p class="mt-1.5 text-sm leading-relaxed text-ink-soft">{{ $v['desc'] ?? '' }}</p>
                </div>
            @endforeach
        </div>
    </div>
...
<section id="surdurulebilirlik" class="relative bg-earth-600">
    <div class="pointer-events-none absolute left-1/2 top-6 -translate-x-1/2 text-center lg:top-8">
        <span class="font-script text-3xl text-white/90 lg:text-4xl">{{ $aboutSustainScript }}</span>
    ...
    <div class="mx-auto grid max-w-6xl grid-cols-1 items-center gap-8 px-5 pb-14 pt-28 lg:grid-cols-2 lg:gap-14 lg:pb-20 lg:pt-32">
        <div>
            <h2 class="text-3xl font-bold text-white lg:text-4xl">{{ $aboutSustainTitle }}</h2>
            <p class="mt-4 max-w-md text-[15px] leading-relaxed text-white/80">{{ $aboutSustainBody }}</p>
            @if(!empty($aboutSustainItems))
            <ul class="mt-6 space-y-3">
                @foreach($aboutSustainItems as $s)
        ...
        <div class="overflow-hidden rounded-md lg:max-w-md">
            <img src="{{ $aboutSustainImage }}" alt="{{ $aboutSustainTitle }}" class="aspect-[4/3] w-full object-cover" loading="lazy" decoding="async">
        </div>
    </div>
</section>
                ...
                <div class="tl-item {{ $left ? 'tl-item--l' : 'tl-item--r' }} relative block pl-12 {{ $loop->last ? '' : 'pb-14 lg:pb-20' }} lg:grid lg:grid-cols-2 lg:gap-0 lg:pl-0">
                    ...
                    <div class="relative {{ $left ? 'lg:pr-14 lg:text-right' : 'lg:pl-14' }}">
                        ...
                        <div class="relative ml-2 rounded-2xl p-6 lg:ml-0 {{ !empty($m['highlight']) ? 'overflow-hidden bg-earth-700' : 'bg-white ring-1 ring-hair' }}">
                            ...
                            <span class="inline-flex items-center gap-2 text-sm font-extrabold uppercase tracking-wide {{ !empty($m['highlight']) ? 'text-leaf-300' : 'text-leaf-600' }} {{ $left ? 'lg:flex-row-reverse' : '' }}">{{ $m['year'] ?? '' }}@if(!empty($m['tag'])) · {{ $m['tag'] }}@endif</span>
...
<section class="bg-earth-700">
    <div class="mx-auto flex max-w-6xl flex-col items-center gap-8 px-5 py-14 text-center lg:flex-row lg:justify-between lg:py-16 lg:text-left">
        <div class="max-w-2xl">
            <h2 class="text-2xl font-extrabold text-white lg:text-3xl">{{ $aboutCtaTitle }}</h2>
            <p class="mt-3 text-base leading-relaxed text-white/80">{{ $aboutCtaText }}</p>
        </div>
