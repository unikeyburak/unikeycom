...
@if($menuItems->isEmpty())
    <a href="{{ $menuUrl }}" class="transition hover:text-white/70" @if($menuIsExternal) target="_blank" @endif>
        {{ $menuLabel }}
    </a>
@else
    {{-- Nav tetikleyici --}}
    <a href="{{ $menuUrl }}" data-mega-btn aria-haspopup="true" aria-expanded="false" aria-controls="mega"
       class="inline-flex items-center gap-1 transition hover:text-white/70" @if($menuIsExternal) target="_blank" @endif>
        {{ $menuLabel }}
    ...
    {{-- Tam ekran mega panel --}}
    <div id="mega" class="fixed inset-0 z-50" aria-hidden="true" role="dialog" aria-label="{{ __('Ürün kategorileri') }}" aria-modal="true">
            ...
            <div class="mx-auto max-w-6xl px-5 py-6 lg:py-8">
                ...
                <div class="flex items-center justify-between border-b border-hair pb-5">
                    <div class="flex items-center gap-2.5">
                        ...
                        <h2 class="text-lg font-extrabold tracking-tight text-ink">{{ __('Ürün Kategorileri') }}</h2>
                    </div>
                    <button type="button" data-mega-close aria-label="{{ __('Kapat') }}" class="grid h-10 w-10 place-items-center rounded-full text-ink-soft transition hover:bg-leaf-500/10 hover:text-leaf-700">
                ...
                {{-- Gövde: sol ana kategoriler + sağ panel --}}
                <div class="grid grid-cols-1 gap-0 lg:grid-cols-[260px_1fr]">
                    <ul class="border-b border-hair py-3 lg:border-b-0 lg:border-r lg:py-5 lg:pr-3" role="tablist" aria-label="{{ __('Ana kategoriler') }}">
                        @foreach($menuItems as $index => $item)
                            <li>
                                <button type="button" role="tab" data-mega-cat="{{ $item['key'] }}"
                                        class="group flex w-full items-center justify-between gap-2 rounded-lg px-4 py-2.5 text-left text-[15px] font-bold transition {{ $index === 0 ? 'active bg-leaf-600 text-white' : 'text-ink hover:bg-leaf-500/10' }}"
                    ...
                    <div class="py-5 lg:pl-8">
                        @foreach($menuItems as $index => $item)
                            <div data-mega-panel="{{ $item['key'] }}" @if($index !== 0) hidden @endif>
                                <div class="mb-5 flex flex-wrap items-end justify-between gap-3">
                                    <div>
                                        <h3 class="text-2xl font-extrabold tracking-tight text-ink">{{ $item['label'] }}</h3>
                                        @if(!empty($item['description']))
                                            <p class="mt-1 max-w-lg text-sm leading-relaxed text-ink-soft line-clamp-2">{{ $item['description'] }}</p>
                                        @endif
                                    </div>
                                    <a href="{{ $item['url'] }}" class="inline-flex shrink-0 items-center gap-1.5 rounded bg-leaf-600 px-4 py-2.5 text-sm font-extrabold text-white transition hover:bg-leaf-700">
                                        {{ __('Tümünü Gör') }}
                                ...
                                @if($item['subcategories']->count() > 0)
                                    <div class="mb-5 flex flex-wrap gap-2">
                                        @foreach($item['subcategories'] as $child)
                                            <a href="{{ $child['url'] }}" class="rounded-full border border-hair px-3.5 py-1.5 text-sm font-semibold text-ink transition hover:border-leaf-400 hover:bg-leaf-50 hover:text-leaf-700">{{ $child['title'] }}</a>
                                        @endforeach
                                    </div>
                                @endif

                                @if($item['promos']->count() > 0)
                                    <div class="grid grid-cols-2 gap-4 sm:grid-cols-3">
                                        @foreach($item['promos'] as $promo)
                                            <a href="{{ $promo['url'] }}" class="reveal-item group block overflow-hidden rounded-2xl bg-white ring-1 ring-hair transition-all hover:-translate-y-1 hover:ring-leaf-400">
                                                <div class="bg-gradient-to-b from-leaf-50/70 to-white p-3">
                                                    <div class="aspect-[5/4] w-full overflow-hidden rounded-[10px] bg-leaf-50">
                                                        @if(!empty($promo['image_path']))
                                                            <x-responsive-image :path="$promo['image_path']" :alt="$promo['title']"
                                                                class="h-full w-full object-cover transition-transform duration-300 group-hover:scale-105"
                                                                sizes="200px" loading="lazy" />
                                                        @elseif(!empty($promo['image']))
                                                            <img src="{{ $promo['image'] }}" alt="{{ $promo['title'] }}" class="h-full w-full object-cover transition-transform duration-300 group-hover:scale-105" loading="lazy">
                                                        @endif
                                                    </div>
                                                </div>
                                                <div class="flex items-center justify-between gap-2 px-4 py-3.5">
                                                    <span class="block font-extrabold text-ink transition group-hover:text-leaf-700">{{ $promo['title'] }}</span>
